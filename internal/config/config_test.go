...
func TestLoadDefaults(t *testing.T) {
	...
	for _, k := range []string{"CODEX_REVIEW_LOOP_MODEL", "CODEX_REVIEW_LOOP_FLAGS", "CODEX_WORKER_FLAGS", "CODEX_CALL_TIMEOUT_SEC", "CLAUDE_PLUGIN_ROOT"} {
		os.Unsetenv(k)
	}

	c := Load()
	if c.CodexModel != "gpt-5.2-codex" {
		t.Errorf("CodexModel = %q, want %q", c.CodexModel, "gpt-5.2-codex")
	}
	...
	if c.CallTimeoutSec != 120 {
		t.Errorf("CallTimeoutSec = %d, want %d", c.CallTimeoutSec, 240)
	}
	if c.PluginRoot != "" {
...
func TestLoadFromEnv(t *testing.T) {
	t.Setenv("CODEX_REVIEW_LOOP_MODEL", "test-model")
...
func TestLoadBadTimeout(t *testing.T) {
	...
	c := Load()
	if c.CallTimeoutSec != 120 {
		t.Errorf("CallTimeoutSec = %d, want 120 for invalid input", c.CallTimeoutSec)
	}
}
